...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    newp->value = old_root->value;
    newp->left = copy_tree(old_root->left);
    newp->right = copy_tree(old_root->right);
    return newp;
  }

  void accumulate(T& sum, const TreeNode<T>* p){
    if(p == NULL)
      return;
    sum += p->value;
    accumulate(sum, p->left);
    accumulate(sum, p->right);
  }
  ...
  iterator find_NRC(const T& key_value, TreeNode<T>* p){
    if (!p)
      return iterator(NULL);
    TreeNode<T> *cur = p;
    while (cur->left != NULL || cur->right != NULL){
      if(cur->left && cur->value > key_value)
        cur = cur->left;
      else if (cur->right && cur->value < key_value)
        cur = cur->right;
      if (!cur->left && !cur->right && cur->value != key_value)
        return iterator(NULL);
      if(cur->value == key_value)
        break;
    }
    return iterator(cur);
  }
