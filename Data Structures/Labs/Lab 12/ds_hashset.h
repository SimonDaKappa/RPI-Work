...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Already at end
    m_size++;
    itr = m_table[index].insert(itr, key);
    return std::pair<iterator,bool>(iterator(this, index, itr), true);
  }
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::vector<std::list<KeyType>> m_tableNew(new_size);
    int c = 0;
    iterator itr = begin();
    while(itr != end()){
      unsigned int hash_value = m_hash(*itr);
      unsigned int index = hash_value % new_size;
      m_tableNew[index].push_back(*itr);
      ++c;
      ++itr;
    }
    m_size = c;
