...
private:
  ...
  void traverse(Balloon* start, std::vector<Balloon*> & visited){
    for (int i = 0; i < start->numRopes(); ++i){
      if (std::find(start->getRope(i), visited.begin(), visited.end()) == visited.end()){
        visited.insert(start->getRope(i));  
        traverse(start->getRope(i), visited);
      }
    }
  } 
