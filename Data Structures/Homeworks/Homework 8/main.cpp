...
void simple_test() {
  ...
  // note: this is a pre-order traversal, the same order as the 
  // 'sideways' tree above!!
...
void student_tests() {
  ...
  // --------------------------------------------------------
  // Different number_type / label_type
  // Golf Shots - Tree is a placement of different golfers and where
  // their ball lands
  QuadTree<float, std::string> golf;
  assert(golf.size() == 0);

  std::vector<std::pair<Point<float>, std::string>> golf_shots;
  golf_shots.push_back(std::make_pair(Point<float>(20.3, 10), "Rory McIlroy"));
  golf_shots.push_back(std::make_pair(Point<float>(10.1, 5), "Phil Mickelson"));
  golf_shots.push_back(std::make_pair(Point<float>(30.123, 4), "Justin Thomas"));
  golf_shots.push_back(std::make_pair(Point<float>(11, 15), "Justin Rose"));
  golf_shots.push_back(std::make_pair(Point<float>(31, 16.18), "Dustin Johnson"));
  golf_shots.push_back(std::make_pair(Point<float>(5, 3), "Jordan Spieth"));
  golf_shots.push_back(std::make_pair(Point<float>(15.954, 2), "Bryson DeChambeau"));
  golf_shots.push_back(std::make_pair(Point<float>(4, 7), "Hideki Matsuyama"));
  golf_shots.push_back(std::make_pair(Point<float>(14.14, 8), "Adam Scott"));
  golf_shots.push_back(std::make_pair(Point<float>(25, 1.34567), "Victor Hovland"));
  golf_shots.push_back(std::make_pair(Point<float>(35, 2), "Tiger Woods"));
  golf_shots.push_back(std::make_pair(Point<float>(26.48345, 7), "John Rahm"));

  for (unsigned int i = 0; i < golf_shots.size(); i ++){
    golf.insert(golf_shots[i].first, golf_shots[i].second);
    assert(golf.size() == i + 1);
  }
  ...
  QuadTree<float, std::string>::iterator itr= golf.begin();
  assert(itr.getLabel() == "Rory McIlroy");
  QuadTree<float, std::string>::iterator itr2(itr);
  assert(itr2 == itr);
  itr2++;
  assert(itr2 != itr);
  QuadTree<float, std::string>::iterator itr3 = itr;
  assert(itr3 == itr);

  // --------------------------------------------------------
  // bf_iterator
  // assignment operator / copy constructor
  QuadTree<float, std::string>::bf_iterator bf_itr = golf.bf_begin();
  assert(bf_itr.getLabel() == "Rory McIlroy");
  bf_itr++;
  bf_itr++;
  assert(bf_itr.getLabel() == "Justin Thomas");
  QuadTree<float, std::string>::bf_iterator bf_itr2(bf_itr);
  assert(bf_itr2 == bf_itr);
  QuadTree<float, std::string>::bf_iterator bf_itr3 = bf_itr;
  assert(bf_itr3 == bf_itr);
  bf_itr3++;
  assert(bf_itr3 != bf_itr);
