...
public:
  ...
  Tile(const std::string &north, const std::string &east,
       const std::string &south, const std::string &west);

  // MODIFIERS
  void rotate(); //Rotate by 90 degrees clock-wise
  void rotateInverse(); //Rotate by 90 degrees counter clock-wise
  ...
  int getRotateC() const { return rotate_count; }
...
std::ostream &operator<<(std::ostream &ostr, const Tile &t);
