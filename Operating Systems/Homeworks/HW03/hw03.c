...
{   
    ...
    for (int i = 0; i < valid_move_count; i++)
    {   
        // Note: this will enforce the order of joining threads to be 
        // Those spawned by clockwise moves from the current position
        // So we have a partial ordering for output
        int joined_move_number;
        int success = pthread_join(threads[i], (void **)&joined_move_number);
#ifdef DEBUG
        printf("DEBUG: THREAD %ld: Thread [%ld] joined (returned %d), success = %d\n", (long)pthread_self(), (long)threads[i], joined_move_number, success);
#endif
        printf("%sTHREAD %ld: Thread [%ld] joined from multiple (returned %d)\n", DEBUG_STR, (long)pthread_self(), (long)threads[i], joined_move_number);
    }
